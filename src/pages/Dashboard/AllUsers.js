...
const AllUsers = () => {
    ...
    const { data: users = [], refetch } = useQuery({
        queryKey: ['users'],
        queryFn: async () => {
            const res = await fetch('https://bookshore-server-shahtaz-tqldd.vercel.appusers')
    ...
    const handleDelete = user => {
        fetch(`https://bookshore-server-shahtaz-tqldd.vercel.appusers/${user._id}`, {
            method: 'DELETE',
            headers: {
    ...
    const handleMakeAdmin = (id) => {
        const url = `https://bookshore-server-shahtaz-tqldd.vercel.appusers/admin/${id}`
        fetch(url, {
            method: 'PUT',
            headers: {
                authorization: `Beared ${localStorage.getItem('accessToken')}`
    ...
    const handleVerifyUser = (id) => {
        const url = `https://bookshore-server-shahtaz-tqldd.vercel.appusers/verify/${id}`
        fetch(url, {
            method: 'PUT',
            headers: {
                authorization: `Beared ${localStorage.getItem('accessToken')}`
